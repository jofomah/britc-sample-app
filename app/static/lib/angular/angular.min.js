...
(function (V, W, v) {
    ...
    function z(b) {
        return function () {
            var a = arguments[0], c, a = "[" + (b ? b + ":" : "") + a + "] http://errors.angularjs.org/1.2.30/" + (b ? b + "/" : "") + a;
    ...
    function ce() {
        this.$get = ["$window", "$document", function (b, a) {
            ...
            {}).userAgent), f = a[0] || {}, g = f.documentMode, h, k = /^(Moz|webkit|O|ms)(?=[A-Z])/, l = f.body && f.body.style, m = !1, n = !1;
            if (l) {
    ...
    var Pe = "validity", A = function (b) {
        ...
        }, u, D, Ea, va = [].slice, Qe = [].push, Aa = Object.prototype.toString, Ua = z("ng"), Wa = V.angular || (V.angular = {}), Za, Na, na = ["0", "0", "0"];
    ...
    var ie = /([\:\-\_]+(.))/g, je = /^moz([A-Z])/, Gb = z("jqLite"), ke = /^<(\w+)\s*\/?>(?:<\/\1>|)$/, Hb = /<|&#?\w+;/, le = /<([\w:]+)/, me =
    ...
    var qe = /^function\s*[^\(]*\(\s*([^\)]*)\)/m, re = /,/, se = /^\s*(_?)(\S+?)\1\s*$/, pe = /((\/\/.*$)|(\/\*[\s\S]*?\*\/))/mg, db = z("$injector"), Re = z("$animate"),
    ...
    var td = ["$animate", function (a) {
        ...
        var d =
            /^\s*([\s\S]+?)(?:\s+as\s+([\s\S]+?))?(?:\s+group\s+by\s+([\s\S]+?))?\s+for\s+(?:([\$\w][\$\w]*)|(?:\(\s*([\$\w][\$\w]*)\s*,\s*([\$\w][\$\w]*)\s*\)))\s+in\s+([\s\S]+?)(?:\s+track\s+by\s+([\s\S]+?))?$/, e = {$setViewValue: B};
        return {
            restrict: "E",
            ...
            link: function (e, g, h, k) {
                ...
                function n(e, f, g) {
                    ...
                    var l = c(k[2] || k[1]),
                        m = k[4] || k[6], n = k[5], r = c(k[3] || ""), A = c(k[2] ? k[1] : m), B = c(k[7]), y = k[8] ? c(k[8]) : null, D = [[{
                            element: f,
                            label: ""
                        }]];
                ...
                if (k[1]) {
                    var q = k[0];
                    k = k[1];
                    var p = h.multiple, s = h.ngOptions, z = !1, A, t = D(W.createElement("option")), x = D(W.createElement("optgroup")),
                        B = t.clone();
                    h = 0;
